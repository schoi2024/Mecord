...
fun DetermineDistribution(spends: DoubleArray): Int {
    ...
    if ((necessities / 4) > pure_wants) {
        return 0 
    } else if ((necessities / 3) < pure_wants) {
        return 1
    } else if (grocery_ratio > (eating_out_ratio * 2)) {
        return 2
    }
    return 1
}
...
fun CalculateHeaviness(time : Int, distribution: Int, cost: Int): DoubleArray {
    ...
    if (distribution == 0) {
        for (i in 0 until time - 1) {
            heaviness[i] = weekly_saving
        }
    } else if (distribution == 1) {
        for (i in instint until (instint * (-1))) {
            if ((time % 2 == 0) && i == 0) {
                continue
            }
            heaviness[i] = weekly_saving * (i * percentage + 100)
        }
    } else {
        for (i in (instint * (-1)) until instint) {
            if ((time % 2 == 0) && i == 0) {
                continue
            }
            heaviness[i] = weekly_saving * (i * percentage + 100)
